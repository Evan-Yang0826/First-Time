...
# 数据库绑定
register_tortoise(app, db_url="mysql://root:admin@localhost:3306/schoollocal",
...
async def stm32track():
    ...
    if (track_pre is not None and len(track_pre) > 20):
        ...
        if device.types == 1:
            ...
            stu = await Student.filter(cardNo=cardstr).first()
            await Track(status=0, stuname=stu.name, location=device.devicelocation, cardno=stu.cardNo,
                        ctime=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())).save()
    return JSONResponse(status_code=200, content={"msg": "success"})
...
@app.post("/api/tracks/delTracks")
async def del_tracks(req: Request):
    tracks = await Track.all()
    if len(tracks) > 0:
        for track in tracks:
